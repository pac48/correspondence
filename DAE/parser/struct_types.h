...
void allocateMesh(Mesh* mesh, int numFaces, int numNormals, int numtextureCoords){
...
void addInput(Controller* controller, unordered_map<int, Input>& controllerInputs, int jointIndex, int weightIndex, int vertInd){
//    int startInd = input->source->stride*index;
//    float* data = (float *) input->source->array;
    string* names;
    float* weights;
