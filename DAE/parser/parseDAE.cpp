...
template<typename T>
void fillArray(const char * data, T* array, int count){
...
void parseTransform(const XMLNode* cur, Node* node){
    ASSERT(cur->ToElement()->FindAttribute("sid") && string(cur->ToElement()->FindAttribute("sid")->Value()) == "transform", "The joint node is formatted incorrectly");
    fillArray<float>(cur->ToElement()->FirstChild()->Value(), node->transform, 16);
}

template<typename T>
void parseArray(const XMLNode* cur, Source* source){
...
void parseTechnique(const XMLNode* cur, Source* source){
    ...
    source->count = count;
    for (const XMLNode* node = childElem->FirstChild(); node; node=node->NextSibling()) {
...
void parseTechnique(const XMLNode* cur, Light* light) {
    ...
    ASSERT(string(childElem->Value()) == "directional", "make sure camera is in perspective mode");
    light->type = "directional";
    for (const XMLNode *child = childElem->FirstChild(); child; child = child->NextSibling()) {
...
void parseTechnique(const XMLNode* cur, Camera* camera) {
    const XMLElement *childElem = cur->FirstChildElement()->ToElement();
    ASSERT(string(childElem->Value()) == "perspective", "make sure camera is in perspective mode");
...
void parseSource(const XMLNode* cur){
    ...
    idMap<Source>[string(curElem->FindAttribute("id")->Value())] = source;

    for (const XMLNode* node = cur->FirstChild(); node; node=node->NextSibling())
    {
...
void parseVertices(const XMLNode* cur){
    ...
    ASSERT(string(childElem->FindAttribute("semantic")->Value()) == "POSITION", "Vertex semantic must be position");
    idMap<Source>[id] = idMap<Source>[sourceID.substr(1, sourceID.size() - 1)];
}

void parseInput2(const XMLNode* cur, unordered_map<int, Input>& inputs, Input& input){
...
template<typename T>
void parseInput(const XMLNode* cur, T& inputs){
    const XMLElement* curElem = cur->ToElement();
    Input input = Input();
    ...
    input.source = idMap<Source>[sourceID.substr(1, sourceID.size() - 1)];
...
void parseJoints(const XMLNode* cur, Controller* controller){
    ...
    for (const XMLNode *child = cur->FirstChild(); child; child = child->NextSibling()) {
    ...
    for (int j = 0 ; j < jointNames->count; j++){
        copy((float*) invBindTransforms->array + j*16, ((float*) invBindTransforms->array) + + (j+1)*16, back_inserter(controller->joint2BindInv[((string*) jointNames->array)[j]]));
    }
}


void parsePrimitives(const XMLNode* cur, unordered_map<int, Input> meshInputs, Mesh* mesh){
...
template<typename T>
void parseTriangles(const XMLNode* cur, T* mesh) {}
...
void parseInfluences(const char* data , int* vcount, int numVerts, Controller* controller, unordered_map<int, Input>& controllerInputs){
...
//                controller->influences[names[i]] = new float[numVerts];
//                memset(controller->influences[names[i]], 0, sizeof(float)*numVerts);
            }

        }
    }

    ASSERT(controllerInputs.size()==2, "There should only be two inputs for the influences");
    stringstream ss(data);
    ...
    for (const XMLNode *child = cur->FirstChild(); child; child = child->NextSibling()) {
        switch (getCase(child->Value())) {
            ...
            case VCOUNT:
                ...
                break;
            case INFLUENCES:
                parseInfluences(child->FirstChild()->Value(), vcount, numVerts,  controller, controllerInputs);
//            case PRIMITIVE:
...
void instantiateCamera(Node* node, string url){
    Camera* camera = idMap<Camera>[url.substr(1, url.size() - 1)];
    memcpy(camera->transform, node->transform, sizeof(float)*16);
}

void instantiateLight(Node* node, string url){
    Light* light = idMap<Light>[url.substr(1, url.size() - 1)];
    for (int i = 0 ; i < 3; i++){
...
void instantiateController(const XMLNode* cur, Node* node, string url){
    ...
    Controller* controller = idMap<Controller>[url.substr(1, url.size() - 1)];
    ...
    for (const XMLNode *child = cur->FirstChild(); child; child = child->NextSibling()) {
        if (string(child->Value()) == "skeleton"){
            ...
            controller->skeletons.push_back(idMap<Node>[skeletonURL.substr(1, skeletonURL.size() - 1)]);
        }
...
template<typename T>
void parseNode(const XMLNode* cur, T parent){}
...
template<typename T>
void parseObjectRecurse(const XMLNode* cur, T* object){
...
template<typename T>
void setIDAndType(const XMLNode* cur, T &object){}
...
template<typename T>
void parseObject(const XMLNode* cur){
    auto id = cur->ToElement()->FindAttribute("id")->Value();
    T object;

    setIDAndType(cur, object);

    parseObjectRecurse(cur, &object);

    objects<T>.push_back(object);
    idMap<T>[id] = &objects<T>.back();
}
...
void recurse(const XMLNode* cur){
    if (cur){
        switch (getCase(cur->Value())) {
            ...
            case GEOMETRY:
                parseObject<Mesh>(cur);
                break;
            case CAMERA:
                parseObject<Camera>(cur);
                break;
            case LIGHT:
                parseObject<Light>(cur);
                break;
            case CONTROLLER:
                parseObject<Controller>(cur);
                break;
            case NODE:
                parseObject<Node>(cur);
                break;
