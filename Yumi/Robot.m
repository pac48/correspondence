...
obj.jointMaximums = obj.jointMaximums - .25;
            ...
            for b = 1:length(obj.bodyNames)
                ...
                obj.bodies{b} = obj.verts{find(tmp,1)};
            end

            obj.setJoints(obj.home)
            obj.startInd = 1;

            tmp = load([pwd '\correspondenceNet\inds_yumi.mat']);
            indsYumi = tmp.inds; % inds specifying alligned arms verts
            [~, ~, ~, robotArmInds]  = obj.getRobotVerts();
            obj.armInds = robotArmInds(indsYumi);
...
%             verts = verts(obj.armInds, :);
%             for i =1:size(obj.indsYumi,2)
%                 v = verts(obj.indsYumi(:,i),:);
...
%             end
        ...
        function JVerts = getVertJacobians(obj, verts)
            ...
            for j = 1:length(JVerts)
                ...
                J = zeros(3, obj.numJoints);
%                 T = obj.getTransform(b);
                Ptip = verts(:, j);
                for i = 1:(b-jointOffset)
                    % right
                    if contains(obj.bodyNames{i+jointOffset}, 'gripper')
                        continue
                    end
